...
/** Колико дана уназад трака показује. Даље од тога човек ретко гледа. */
const UNAZAD = 7;

/**
 * Распон дана који се тражи од сервера.
 *
 * Обично је то последња недеља и све што следи. Али кад нови јеловник
 * касни, последњи дан који постоји је старији и од тога, па доња граница
 * мора да се помери уназад до њега. Без тога доња граница прескочи горњу,
 * упит врати нула дана и апликација остане празна.
 */
function menuWindow(today, first, last) {
  const to = last || shift(today, 14);
  const prag = shift(today, -UNAZAD);
  const donja = prag < to ? prag : shift(to, -UNAZAD);
