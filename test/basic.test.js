...
test('распон дана обухвата последњи јеловник и кад он касни', () => {
  // Кад јеловник закасни више него што трака гледа уназад, доња граница
  // упита прескочи горњу, сервер врати нула дана, а апликација остане
  // празна. Мерено у прегледачу: from 2026-09-12, to 2026-09-11, нула дана.
  const src = fs.readFileSync('./public/app.js', 'utf8');
  const telo = src.slice(src.indexOf('const shift ='), src.indexOf('const weekdayIndex ='))
    + src.slice(src.indexOf('const UNAZAD ='), src.indexOf('async function load'));
  const menuWindow = new Function(`${telo}
; return menuWindow;`)();

  // Обичан дан: последња недеља и све што следи.
  assert.deepEqual(menuWindow('2026-09-15', '2026-09-01', '2026-09-30'),
    { from: '2026-09-08', to: '2026-09-30' });

  // Јеловник касни више од недеље: прозор мора да се помери уназад до њега.
  const kasni = menuWindow('2026-09-25', '2026-09-01', '2026-09-11');
  ...
  assert.equal(kasni.from, '2026-09-04');
